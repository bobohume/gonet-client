...
function bint.new( num ) --{{{2
   local bignum = {} ;
   setmetatable( bignum , bint.mt ) ;
   bint.change( bignum , num ) ;
   return bignum ;
end

--%%%%%%%%%%%%%%%%%%%% Functions for metatable %%%%%%%%%%%%%%%%%%%%--{{{1
--bint.mt.sub{{{2
function bint.mt.sub( num1 , num2 )
   local temp = bint.new() ;
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   bint.sub( bnum1 , bnum2 , temp ) ;
   return temp ;
end

--bint.mt.add{{{2
function bint.mt.add( num1 , num2 )
   local temp = bint.new() ;
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   bint.add( bnum1 , bnum2 , temp ) ;
   return temp ;
end

--bint.mt.mul{{{2
function bint.mt.mul( num1 , num2 )
   local temp = bint.new() ;
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   bint.mul( bnum1 , bnum2 , temp ) ;
   return temp ;
end

--bint.mt.div{{{2
function bint.mt.div( num1 , num2 )
   local bnum1 = {} ;
   local bnum2 = {} ;
   local bnum3 = bint.new() ;
   local bnum4 = bint.new() ;
   bnum1 = bint.new( num1 ) ;
   bnum2 = bint.new( num2 ) ;
   bint.div( bnum1 , bnum2 , bnum3 , bnum4 ) ;
   return bnum3 , bnum4 ;
end

function bint.mt.mod( num1 , num2 )
   local bnum1 = {} ;
   local bnum2 = {} ;
   local bnum3 = bint.new() ;
   local bnum4 = bint.new() ;
...
--bint.mt.tostring{{{2
function bint.mt.tostring( bnum )
   local i = 0 ;
   local j = 0 ;
   local str = "" ;
   local temp = "" ;
...
--bint.mt.pow{{{2
function bint.mt.pow( num1 , num2 )
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   return bint.pow( bnum1 , bnum2 ) ;
end

--bint.mt.eq{{{2
function bint.mt.eq( num1 , num2 )
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   return bint.eq( bnum1 , bnum2 ) ;
end

--bint.mt.lt{{{2
function bint.mt.lt( num1 , num2 )
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   return bint.lt( bnum1 , bnum2 ) ;
end

--bint.mt.le{{{2
function bint.mt.le( num1 , num2 )
   local bnum1 = bint.new( num1 ) ;
   local bnum2 = bint.new( num2 ) ;
   return bint.le( bnum1 , bnum2 ) ;
end

--bint.mt.unm{{{2
function bint.mt.unm( num )
   local ret = bint.new( num )
   if ret.signal == '+' then
      ret.signal = '-'
   else
      ret.signal = '+'
...
bint.mt.__metatable = "hidden"           ; -- answer to getmetatable(aBignum)
-- bint.mt.__index     = "inexistent field" ; -- attempt to acess nil valued field 
-- bint.mt.__newindex  = "not available"    ; -- attempt to create new field
bint.mt.__tostring  = bint.mt.tostring ;
-- arithmetics
bint.mt.__add = bint.mt.add ;
bint.mt.__sub = bint.mt.sub ;
bint.mt.__mul = bint.mt.mul ;
bint.mt.__div = bint.mt.div ;
bint.mt.__mod = bint.mt.mod ;
bint.mt.__pow = bint.mt.pow ;
bint.mt.__unm = bint.mt.unm ;
-- Comparisons
bint.mt.__eq = bint.mt.eq   ; 
bint.mt.__le = bint.mt.le   ;
bint.mt.__lt = bint.mt.lt   ;

--concatenation
-- bint.me.__concat = ???

setmetatable( bint.mt, { __index = "inexistent field", __newindex = "not available", __metatable="hidden" } ) ;
...
--  %%%%%%%% --
--Funcao bint.add{{{2
function bint.add( bnum1 , bnum2 , bnum3 )
   local maxlen = 0 ;
   local i = 0 ;
   local carry = 0 ;
   local signal = '+' ;
   ...
   if bnum1 == nil or bnum2 == nil or bnum3 == nil then
      error("Function bint.add: parameter nil") ;
   elseif bnum1.signal == '-' and bnum2.signal == '+' then
      bnum1.signal = '+' ;
      bint.sub( bnum2 , bnum1 , bnum3 ) ;
...
--  %%%%%%%% --
--Funcao bint.sub{{{2
function bint.sub( bnum1 , bnum2 , bnum3 )
   local maxlen = 0 ;
   local i = 0 ;
   local carry = 0 ;
   local old_len = 0 ;
   ...
   if bnum1 == nil or bnum2 == nil or bnum3 == nil then
      error("Function bint.sub: parameter nil") ;
   elseif bnum1.signal == '-' and bnum2.signal == '+' then
      bnum1.signal = '+' ;
      bint.add( bnum1 , bnum2 , bnum3 ) ;
      bnum3.signal = '-' ;
      if not rawequal(bnum1, bnum3) then
         bnum1.signal = '-' ;
      end
   ...
   if bint.compareAbs( bnum1 , bnum2 ) == 2 then
      bint.sub( bnum2 , bnum1 , bnum3 ) ;
      bnum3.signal = '-' ;
      return 0 ;
   else
...
function bint.mul( bnum1 , bnum2 , bnum3 )
   local i = 0 ; j = 0 ;
   local temp = bint.new( ) ;
   local temp2 = 0 ;
   local carry = 0 ;
   ...
   if bnum1 == nil or bnum2 == nil or bnum3 == nil then
      error("Function bint.mul: parameter nil") ;
   --Handle the signals
   elseif bnum1.signal ~= bnum2.signal then
      bint.mul( bnum1 , -bnum2 , bnum3 ) ;
      bnum3.signal = '-' ;
      return 0 ;
   end
...
--  %%%%%%%% --
--bint.div{{{2
function bint.div( bnum1 , bnum2 , bnum3 , bnum4 )
   local temp = bint.new() ;
   local temp2 = bint.new() ;
   local one = bint.new( "1" ) ;
   local zero = bint.new( "0" ) ;
   --Check division by zero
   if bint.compareAbs( bnum2 , zero ) == 0 then
      error( "Function bint.div: Division by zero" ) ;
   end     
   --Handle the signals
   if bnum1 == nil or bnum2 == nil or bnum3 == nil or bnum4 == nil then
      error( "Function bint.div: parameter nil" ) ;
   elseif bnum1.signal == "+" and bnum2.signal == "-" then
      bnum2.signal = "+" ;
      bint.div( bnum1 , bnum2 , bnum3 , bnum4 ) ;
      bnum2.signal = "-" ;
   ...
   elseif bnum1.signal == "-" and bnum2.signal == "+" then
      ...
      if bnum4 < zero then --Check if remainder is negative
         bint.add( bnum3 , one , bnum3 ) ;
         bint.sub( bnum2 , bnum4 , bnum4 ) ;
      end
      bnum3.signal = "-" ;
      return 0 ;
   elseif bnum1.signal == "-" and bnum2.signal == "-" then
   ...
   bint.copy( bnum1 , bnum4 ) ;

   --Check if can continue dividing
   while( bint.compareAbs( bnum4 , bnum2 ) ~= 2 ) do
      if bnum4[bnum4.len - 1] >= bnum2[bnum2.len - 1] then
         bint.put( temp , math.floor( bnum4[bnum4.len - 1] / bnum2[bnum2.len - 1] ) , bnum4.len - bnum2.len ) ;
         temp.len = bnum4.len - bnum2.len + 1 ;
      else
         bint.put( temp , math.floor( ( bnum4[bnum4.len - 1] * RADIX + bnum4[bnum4.len - 2] ) / bnum2[bnum2.len -1] ) , bnum4.len - bnum2.len - 1 ) ;
      ...
      bint.add( temp , bnum3 , bnum3 )  ;
      temp = temp * bnum2 ;
      bint.sub( bnum4 , temp , bnum4 ) ;
   end

   --Update if the remainder is negative
   if bnum4.signal == '-' then
      decr( bnum3 ) ;
      bint.add( bnum2 , bnum4 , bnum4 ) ;
   end
   return 0 ;
end
...
--  %%%%%%%% --
--bint.exp{{{2
function bint.pow( bnum1 , bnum2 )
   local n = bint.new( bnum2 ) ;
   local y = bint.new( 1 ) ;
   local z = bint.new( bnum1 ) ;
   local zero = bint.new( "0" ) ;
   if bnum2 < zero then
      error( "Function bint.exp: domain error" ) ;
...
--  %%%%%%%% --
--bint.eq{{{2
function bint.eq( bnum1 , bnum2 )
   if bint.compare( bnum1 , bnum2 ) == 0 then
      return true ;
   else
      return false ;
   end
...
--  %%%%%%%% --
--bint.lt{{{2
function bint.lt( bnum1 , bnum2 )
   if bint.compare( bnum1 , bnum2 ) == 2 then
      return true ;
   else
      return false ;
   end
...
--  %%%%%%%% --
--bint.le{{{2
function bint.le( bnum1 , bnum2 )
   local temp = -1 ;
   temp = bint.compare( bnum1 , bnum2 )
...
--  %%%%%%%% --
--bint.compareAbs{{{2
function bint.compareAbs( bnum1 , bnum2 )
...
--  %%%%%%%% --
--bint.compare{{{2
function bint.compare( bnum1 , bnum2 )
   local signal = 0 ;
   
   if bnum1 == nil or bnum2 == nil then
      error("Funtion bint.compare: parameter nil") ;
   elseif bnum1.signal == '+' and bnum2.signal == '-' then
      return 1 ;
   elseif bnum1.signal == '-' and bnum2.signal == '+' then
      return 2 ;
...
--bint.copy{{{2
function bint.copy( bnum1 , bnum2 )
   if bnum1 ~= nil and bnum2 ~= nil then
      ...
      error("Function bint.copy: parameter nil") ;
   end
end
...
--
--  Description:
--     Changes the value of a bint.
--     This function is called by bint.new.
--
--  Parameters:
--     bnum1, bnum2 - numbers
--
...
--  %%%%%%%% --
--bint.change{{{2
function bint.change( bnum1 , num )
   local j = 0 ;
   local len = 0  ;
   local num = num ;
   local l ;
   local oldLen = 0 ;
   if bnum1 == nil then
      error( "bint.change: parameter nil" ) ;
   elseif type( bnum1 ) ~= "table" then
      error( "bint.change: parameter error, type unexpected" ) ;
   elseif num == nil then
      bnum1.len = 1 ;
      bnum1[0] = 0 ;
      bnum1.signal = "+";
   ...
   elseif type( num ) == "string" or type( num ) == "number" then
      ...
      if sf ~= nil then
         ...
         if e ~= nil and e > 0 then 
            ...
            error( "Function bint.change: string is not a valid number" ) ;
         end
      ...
      error( "Function bint.change: parameter error, type unexpected" ) ;
   end
...
--after with nil.
function bint.put( bnum , int , pos )
   if bnum == nil then
      error("Function bint.put: parameter nil") ;
   end
   local i = 0 ;
   for i = 0 , pos - 1 do
      bnum[i] = 0 ;
